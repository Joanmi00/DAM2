package ExamplesProgramacioConcurrent.com.psp.sumatori;
...
public BufferedReader run(Long n1, Long n2) {
  /*
   * Aquesta classe s'encarrega d'invocar la classe com.psp.sumatori.Suma en un
   * procés, i retorna un enter amb la suma. Per tal de comunicar-se amb Suma, com
   * que aquesta escriu per la seua eixida estàndard el resultat, l'haurem de
   * llegir amb el mètode getInputStream de la classe Process i processar-lo com
   * un Stream d'entrada, com hem vist anteriorment.
   */
  ProcessBuilder pb;
  try {
    // Creem un objecte de la classe processBuilder
    pb = new ProcessBuilder("java", "com.psp.sumatori.Suma", n1.toString(), n2.toString());
...
public long readFromBuffer(BufferedReader br) {
  /*
   * Nova funció que llegeix del BufferedReader que li passem com a paràmetre i
   * retorna l'eixida del procés en format Long.
   */
  try {
