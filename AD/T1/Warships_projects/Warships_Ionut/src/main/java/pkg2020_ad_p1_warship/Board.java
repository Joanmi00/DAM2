...
private static int getBOARD_DIM = 10;
...
public Board() {
  ...
  cells = new Cell[getBOARD_DIM][getBOARD_DIM];
  // inicializa la matriz a agua
  for (int i = 0; i < getBOARD_DIM; i++) {
    for (int j = 0; j < getBOARD_DIM; j++) {
...
private void testEnd() {
  for (Boat boat : boats) {
    if (boat.getBoatState() != Boat.getBoatSunken())
      return;
  }
  end_game = true;
}
...
public int fitValueToBoard(int value) {
  if (value <= 0)
    return 0;
  if (value > getBOARD_DIM - 1)
    return getBOARD_DIM - 1;
  return value;
}
