...
public ResultadoCarrera(Element result) {
  ...
  this.d = new Driver(driver);
  
  Element cons = (Element) result.getElementsByTagName("Constructor").item(0);
  this.constructor = cons.getElementsByTagName("Name").item(0).getTextContent();
  
  this.initialPos = Integer.parseInt(result.getElementsByTagName("Grid").item(0).getTextContent());
  
  //System.out.println(d + " sale de " + this.initialPos + " con un " + this.constructor);
  ...
  int statu = Integer.parseInt(status.getAttribute("statusId"));
  
  if (statu == 1) {
    this.finisher = true;
    Element time = (Element) result.getElementsByTagName("Time").item(0);
    this.timeMillis = Long.parseLong(time.getAttribute("millis"));
  } else {
    this.finisher = false;
    this.timeMillis = Long.MAX_VALUE; // por las ordenaciones
    
  }
  
  this.completedLaps = Integer.parseInt(result.getElementsByTagName("Laps").item(0).getTextContent());
  
  Element fastLap = (Element) result.getElementsByTagName("FastestLap").item(0);
  this.rankFastesLap = Integer.parseInt(fastLap.getAttribute("rank"));
}
