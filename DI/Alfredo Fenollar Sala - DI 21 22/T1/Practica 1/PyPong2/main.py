if __name__ == '__main__':
    ...
    def check_collision():
        ...
        if ball.x <= paddle1.paddle_width or ball.x >= paddle1.paddle_width and (
                ball.y <= paddle1.paddle_height or ball.y >= paddle1.paddle_height):
            print("toque")
    ...
    # Draw the walls
    # pygame.draw.rect(config.screen, config.fgColor, pygame.Rect(0, 0, BORDER, config.HEIGHT))
    # pygame.draw.rect(config.screen, config.fgColor, pygame.Rect((config.WIDTH - BORDER, 0), (config.WIDTH, config.HEIGHT)))
    ...
    paddle1 = Paddle(config.PADDLE_WIDTH, config.HEIGHT // 2)
    paddle1.draw(config.color_p1)
    paddle2 = Paddle(config.WIDTH - config.PADDLE_WIDTH,
                     config.HEIGHT // 2)
    paddle2.draw(config.color_p2)

    # Create and draw the ball
    ball = Ball(config.RADIUS + config.PADDLE_WIDTH, config.HEIGHT // 2, config.VELOCITY,
